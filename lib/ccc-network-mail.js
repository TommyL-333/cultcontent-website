...
// Closes the loop after a brand reviews a challenge entry. Uses notify_message
// as the opt-out: it's the closest existing preference to "a brand contacted
// me about my work", and adding a fourth toggle for one email is noise.
async function sendChallengeReviewEmail(creator, { brand, challenge, status, brandNote }) {
  if (!creator.notify_message) return { ok: true, skipped: 'opted_out' };
  const brandName = brand.brand_name || `${brand.first_name} ${brand.last_name || ''}`.trim();

  const headline = {
    accepted: `${brandName} accepted your entry`,
    winner: `You won ${brandName}'s challenge`,
    rejected: `${brandName} reviewed your entry`,
    submitted: `${brandName} reopened your entry`,
  }[status];

  const body = {
    accepted: `<p><strong>${brandName}</strong> accepted your entry for &ldquo;${challenge.title}&rdquo;.</p>`,
    winner: `<p><strong>${brandName}</strong> picked your entry as the winner of &ldquo;${challenge.title}&rdquo;.${challenge.reward ? ` Reward: ${challenge.reward}.` : ''}</p>`,
    rejected: `<p><strong>${brandName}</strong> reviewed your entry for &ldquo;${challenge.title}&rdquo; and isn&rsquo;t taking it forward this time.</p>`,
    submitted: `<p><strong>${brandName}</strong> reopened your entry for &ldquo;${challenge.title}&rdquo;.</p>`,
  }[status];

  return sendEmail(creator, {
    logLabel: 'challenge-review',
    subject: headline,
    html: `<p>Hey ${creator.first_name},</p>${body}${brandNote ? `<p><em>&ldquo;${brandNote}&rdquo;</em></p>` : ''}<p><a href="${PUBLIC_BASE_URL}/ccc-network/challenges">See it on the roster</a>.</p>`,
  });
}

// Sent when a brand is actually added to the matchmaking campaign, so the
// same "here's how to accept" message isn't retyped for every brand.
async function sendTikTokShopInviteEmail(brand) {
  const name = brand.brand_name || brand.first_name;
  return sendEmail(brand, {
    logLabel: 'tiktok-shop-invite',
    subject: `${name} — you're in the Creator Carnival matchmaking campaign`,
    html: `<p>Hey ${brand.first_name},</p>
<p>We've added your TikTok Shop to the Creator Carnival creator matchmaking campaign. There's an invite waiting for you to accept:</p>
<ol>
  <li>Open <strong>TikTok Shop Seller Center</strong> and go to <strong>Affiliate &rarr; Campaigns</strong>.</li>
  <li>Find the <strong>Creator Carnival</strong> campaign under your invitations.</li>
  <li>Review the commission and <strong>accept</strong> it.</li>
</ol>
<p>Once you accept, creators on the roster can pull your products into their content and tag them live from the floor.</p>
<p>Stuck on any of it, reply to this email and we'll walk you through it.</p>`,
  });
}
...
module.exports = {
  ...
  sendCreatorFormWelcomeEmail, sendChallengeReviewEmail, sendTikTokShopInviteEmail,
  magicLinkUrl,
};
