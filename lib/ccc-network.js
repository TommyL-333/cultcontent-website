...
// booth_note carries the free-text answer when booth_zone is 'other', which
// covers sponsors, activation partners, and people attending without a booth.
...
// ─── Signup / profile ──────────────────────────────────────────────────────────
function signup({ role, first_name, last_name, email, phone, brand_name, tiktok_handle, instagram_handle, category, bio, links, looking_for, tier, terms_accepted, share_contact, booth_zone, booth_note }) {
  ...
  if (existing) {
    ...
    updateProfile(existing.id, { first_name, last_name, phone, brand_name, tiktok_handle, instagram_handle, category, bio, links, looking_for, booth_zone, booth_note });
  ...
  db.prepare(`
    INSERT INTO ccc_people (uuid, role, tier, first_name, last_name, email, phone, brand_name, tiktok_handle, instagram_handle, category, bio, links, looking_for, share_contact, terms_accepted_at, booth_zone, booth_note)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(uuid, role, declaredTier, first_name, last_name || '', email.toLowerCase().trim(), phone || '', brand_name || '', tiktok_handle || '', instagram_handle || '', category || '', bio || '', JSON.stringify(links || []), looking_for || '', share_contact ? 1 : 0, new Date().toISOString(), booth_zone || '', (booth_note || '').trim().slice(0, 300));
...
function updateProfile(id, { first_name, last_name, phone, brand_name, tiktok_handle, instagram_handle, category, bio, links, looking_for, photo_url, rate_videos, rate_price, rate_terms, booth_zone, booth_note }) {
  db.prepare(`
    UPDATE ccc_people SET
      ...
      rate_videos = ?, rate_price = ?, rate_terms = ?, booth_zone = ?, booth_note = ?
    WHERE id = ?
  `).run(first_name || '', last_name || '', phone || '', brand_name || '', tiktok_handle || '', instagram_handle || '',
        ...
        booth_zone || '', (booth_note || '').trim().slice(0, 300), id);
  return getPerson(id);
}
...
function listDirectory(forPerson) {
  ...
  const rows = db.prepare(`
    SELECT uuid, role, first_name, last_name, brand_name, tiktok_handle, instagram_handle, category, bio, links, looking_for, photo_url, booth_zone, booth_note
