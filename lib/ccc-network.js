...
function signup({ role, first_name, last_name, email, phone, brand_name, handle, category, bio, links, looking_for, tier }) {
  ...
  if (existing) {
    ...
    updateProfile(existing.id, { first_name, last_name, phone, brand_name, handle, category, bio, links, looking_for });
...
function listConnections(personId) {
  const rows = db.prepare(`
    SELECT c.*,
      pf.uuid as from_uuid, pf.first_name as from_first, pf.last_name as from_last, pf.role as from_role, pf.brand_name as from_brand, pf.handle as from_handle, pf.category as from_category,
      pt.uuid as to_uuid, pt.first_name as to_first, pt.last_name as to_last, pt.role as to_role, pt.brand_name as to_brand, pt.handle as to_handle, pt.category as to_category
  ...
  const other = (r) => r.from_person_id === personId
    ? { uuid: r.to_uuid, first_name: r.to_first, last_name: r.to_last, role: r.to_role, brand_name: r.to_brand, handle: r.to_handle, category: r.to_category }
    : { uuid: r.from_uuid, first_name: r.from_first, last_name: r.from_last, role: r.from_role, brand_name: r.from_brand, handle: r.from_handle, category: r.from_category };
