...
// Where a brand is on site. Public — a creator planning their route wants it.
addColumnIfMissing('ccc_people', `booth_zone TEXT`);
// booth_note held a free-text follow-up when booth_zone was 'other'. The
// field was removed from the forms; the column stays because dropping it
// would destroy anything already entered, and nothing writes to it now.
addColumnIfMissing('ccc_people', `booth_note TEXT`);

// Whitelisted so a crafted request can't park arbitrary text in a field that
// renders on every other member's screen. Ids match lib/ccc-booths.js where
// they overlap — `freedom-way` is the historical id for American Way.
// Optional, brands only. A TikTok Shop code is a private business
// identifier — it exists so we can invite them to the creator matchmaking
// campaign, and it is never shown to another member.
addColumnIfMissing('ccc_people', `tiktok_shop_code   TEXT NOT NULL DEFAULT ''`);
addColumnIfMissing('ccc_people', `tiktok_shop_status TEXT NOT NULL DEFAULT ''`); // '' | 'submitted' | 'invited'
addColumnIfMissing('ccc_people', `tiktok_shop_at     TEXT`);

const BOOTH_ZONES = ['freedom-way', 'capitol-canopy', 'belvedere', 'other'];
const cleanBoothZone = (z) => (BOOTH_ZONES.includes(z) ? z : '');
...
// Photo URLs saved before share-link rewriting existed are still raw Drive
// /view or Dropbox ?dl=0 links, which never render. Re-normalise them once on
// boot so those profiles fix themselves rather than needing every member to
// go and re-save. normalizePhotoUrl is stable on an already-direct URL, so
// this is a no-op on every subsequent start.
function backfillPhotoUrls() {
  const rows = db.prepare(`SELECT id, photo_url FROM ccc_people WHERE photo_url IS NOT NULL AND photo_url != ''`).all();
  const update = db.prepare(`UPDATE ccc_people SET photo_url = ? WHERE id = ?`);
  let changed = 0;
  for (const row of rows) {
    const next = normalizePhotoUrl(row.photo_url);
    if (next !== row.photo_url) { update.run(next, row.id); changed += 1; }
  }
  if (changed) console.log(`[ccc-network] rewrote ${changed} photo URL(s) to direct-image form`);
}
...
//
// keepContact re-admits email and phone (never rates, never ghl_contact_id)
// for the one case that has its own consent: a brand that ticked "creators
// can contact me directly".
function withoutPrivateFields(person, { keepContact = false } = {}) {
  if (!person) return person;
  const {
    email, phone, ghl_contact_id, rate_videos, rate_price, rate_terms,
    // Never released to another member, connected or not — unlike contact
    // details there is no case where a peer needs a brand's Shop code.
    tiktok_shop_code, tiktok_shop_status, tiktok_shop_at,
    ...rest
  } = person;
  return keepContact ? { ...rest, email, phone } : rest;
}

// ─── Signup / profile ──────────────────────────────────────────────────────────
function signup({ role, first_name, last_name, email, phone, brand_name, tiktok_handle, instagram_handle, category, bio, links, looking_for, tier, terms_accepted, share_contact, booth_zone }) {
  ...
  if (existing) {
    ...
    updateProfile(existing.id, { first_name, last_name, phone, brand_name, tiktok_handle, instagram_handle, category, bio, links, looking_for, booth_zone });
  ...
  db.prepare(`
    INSERT INTO ccc_people (uuid, role, tier, first_name, last_name, email, phone, brand_name, tiktok_handle, instagram_handle, category, bio, links, looking_for, share_contact, terms_accepted_at, booth_zone)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(uuid, role, declaredTier, first_name, last_name || '', email.toLowerCase().trim(), phone || '', brand_name || '', tiktok_handle || '', instagram_handle || '', category || '', bio || '', JSON.stringify(links || []), looking_for || '', share_contact ? 1 : 0, new Date().toISOString(), cleanBoothZone(booth_zone));
...
// Updates only the fields actually present in the payload.
//
// This used to write every column unconditionally, so any caller sending a
// partial body silently blanked everything it omitted — a profile save that
// forgot to include rates wiped the creator's rates. The form always sent the
// full set, which made the hazard invisible right up until something didn't.
const PROFILE_FIELDS = {
  ...
  brand_name: (v) => v || '',
  tiktok_handle: (v) => v || '',
  instagram_handle: (v) => v || '',
  category: (v) => v || '',
  bio: (v) => v || '',
  looking_for: (v) => v || '',
  links: (v) => JSON.stringify(v || []),
  photo_url: (v) => normalizePhotoUrl(v),
  rate_videos: (v) => v || '',
  rate_price: (v) => v || '',
  rate_terms: (v) => v || '',
  booth_zone: (v) => cleanBoothZone(v),
  tiktok_shop_code: (v) => String(v || '').trim().replace(/[^A-Za-z0-9._-]/g, '').slice(0, 64),
};

function updateProfile(id, patch = {}) {
  const keys = Object.keys(PROFILE_FIELDS).filter((k) => patch[k] !== undefined);
  if (!keys.length) return getPerson(id);

  const setSql = keys.map((k) => `${k} = ?`).join(', ');
  const values = keys.map((k) => PROFILE_FIELDS[k](patch[k]));
  db.prepare(`UPDATE ccc_people SET ${setSql} WHERE id = ?`).run(...values, id);
  return getPerson(id);
}
...
//
// People paste the link they have, which is almost always a *share page*
// rather than an image: a Drive /view URL and a Dropbox ?dl=0 URL both serve
// HTML, so <img> fails and the avatar silently falls back. Rewrite the two
// formats that can be rewritten rather than making everyone hunt for a
// "direct" link they've no reason to know about.
function toDirectImageUrl(parsed) {
...
// Photos uploaded through the app live on the DATA_DIR volume and are
// referenced by a relative path, which the external-URL parsing below would
// reject. Matched strictly against the exact shape the upload route writes —
// anything looser and a crafted value could point the <img> at another path.
const UPLOADED_PHOTO_RE = /^\/uploads\/ccc-avatars\/[a-f0-9]{32}\.jpg$/;

function normalizePhotoUrl(raw) {
  const text = (raw || '').trim();
  if (!text) return '';
  if (UPLOADED_PHOTO_RE.test(text)) return text;
  const withScheme = /^https?:\/\//i.test(text) ? text : `https://${text}`;
  try {
    const parsed = new URL(withScheme);
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') return '';
    if (!parsed.hostname.includes('.')) return '';
    return toDirectImageUrl(parsed).slice(0, 500);
  } catch { return ''; }
}
...
function listDirectory(forPerson) {
  ...
  const rows = db.prepare(`
    SELECT uuid, role, first_name, last_name, brand_name, tiktok_handle, instagram_handle, category, bio, links, looking_for, photo_url, booth_zone
...
function getPersonProfile(viewerId, uuid) {
  ...
  // A brand can publish its contact details to creators without a connection.
  // Scoped to creators on purpose — a competing brand still has to connect,
  // and this is not a roster-wide email dump.
  const viewer = getPerson(viewerId);
  const brandOpenToCreators = target.role === 'brand' && !!target.share_contact && viewer?.role === 'creator';
  // An accepted connection releases contact details and rates, but never the
  // TikTok Shop code — that is between the brand and our team, and a peer has
  // no reason to see it at any connection state. Stripping it separately
  // rather than returning the raw row is what keeps that true.
  const { tiktok_shop_code, tiktok_shop_status, tiktok_shop_at, ...connectedView } = target;
  const person = relationship === 'accepted'
    ? connectedView
    : withoutPrivateFields(target, { keepContact: brandOpenToCreators });
  return { ...person, relationship, contact_is_public: brandOpenToCreators && relationship !== 'accepted' };
}
...
// Retained only so an accidental caller fails loudly rather than silently
// granting access. Tier changes go through setStatus from /api/admin/*.
function setTierRequest() {
  return { ok: false, error: 'tier_is_admin_assigned' };
}
...
// Tracks the manual step: a brand submits a code, someone adds them to the
// campaign in TikTok Shop, then marks them invited here. Without a status
// there is nothing to tell you who is still waiting.
const TIKTOK_SHOP_STATUSES = ['', 'submitted', 'invited'];

function setTikTokShopStatus(uuid, status) {
  if (!TIKTOK_SHOP_STATUSES.includes(status)) return { ok: false, error: 'bad_status' };
  const person = getPerson(uuid);
  if (!person) return { ok: false, error: 'not_found' };
  db.prepare(`UPDATE ccc_people SET tiktok_shop_status = ?, tiktok_shop_at = ? WHERE uuid = ?`)
    .run(status, new Date().toISOString(), uuid);
  return { ok: true, person: getPerson(uuid) };
}
...
module.exports = {
  ...
  profileCompletion, normalizePhotoUrl, UPLOADED_PHOTO_RE,
  setTikTokShopStatus,
  PRIORITY_TIERS,
